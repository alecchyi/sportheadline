...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = NSLocalizedString(@"Top Headlines", @"Top Headlines");
...
- (void)refreshData{
    ...
    if ([[[DataService sharedService].messages objectForKey:kTopCount] intValue] > [[[DataService sharedService].messages objectForKey:kTopMsg] count] - 1) {
        self.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",[[[DataService sharedService].messages objectForKey:kTopCount] intValue] - [[[DataService sharedService].messages objectForKey:kTopMsg] count] + 1];
    }else{
...
- (void)reloadMessage{
    ...
    [downloader startDownload:kTop withType:@"top" page:page];
    [downloader release];
}
...
- (void)viewDidLoad
{
    ...
    [downloader startDownload:kTop withType:@"top" page:page];
    [downloader release];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData) name:@"topMessage" object:nil];
...
{
    NSMutableArray *arr = [[DataService sharedService].messages objectForKey:kTopMsg];
    return arr.count ;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Message *message = [[[DataService sharedService].messages objectForKey:kTopMsg] objectAtIndex:indexPath.row];
    BOOL b = NO;
    ...
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kTopMsg] count]) {
...
{
	Message* msg = [[[DataService sharedService].messages objectForKey:kTopMsg] objectAtIndex:indexPath.row];
...
{
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kTopMsg] count]  && isMore){
        if ([[[DataService sharedService].messages objectForKey:kTopCount] intValue] > [[[DataService sharedService].messages objectForKey:kTopMsg] count] - 1) {
            page++;
            activity = [[MBProgressHUD alloc] initWithView:self.view];
            activity.center = self.view.center;
    ...
    }else if(indexPath.row + 1 < [[[DataService sharedService].messages objectForKey:kTopMsg] count]){
