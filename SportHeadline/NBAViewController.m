...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = NSLocalizedString(@"NBA Headlines", @"NBA Headlines");
...
- (void)refreshData{
    ...
    if ([[[DataService sharedService].messages objectForKey:kNBACount] intValue] > [[[DataService sharedService].messages objectForKey:kNBAMsg] count] - 1) {
        self.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",[[[DataService sharedService].messages objectForKey:kNBACount] intValue] - [[[DataService sharedService].messages objectForKey:kNBAMsg] count] + 1];
    }else{
...
- (void)reloadMessage{
    ...
    [downloader startDownload:kNBA withType:@"nba" page:page];
    [downloader release];
}
...
- (void)viewDidLoad
{
    ...
    [downloader startDownload:kNBA withType:@"nba" page:page];
    [downloader release];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData) name:@"nbaMessage" object:nil];
...
{
    NSMutableArray *arr = [[DataService sharedService].messages objectForKey:kNBAMsg];
    return arr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Message *message = [[[DataService sharedService].messages objectForKey:kNBAMsg] objectAtIndex:indexPath.row];
    BOOL b = NO;
    ...
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kNBAMsg] count]) {
...
{
	Message* msg = [[[DataService sharedService].messages objectForKey:kNBAMsg] objectAtIndex:indexPath.row];
...
{
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kNBAMsg] count]  && isMore){
        if ([[[DataService sharedService].messages objectForKey:kNBACount] intValue] > [[[DataService sharedService].messages objectForKey:kNBAMsg] count] - 1) {
            page++;
            activity = [[MBProgressHUD alloc] initWithView:self.view];
            activity.center = self.view.center;
    ...
    }else if(indexPath.row + 1 < [[[DataService sharedService].messages objectForKey:kNBAMsg] count]){
