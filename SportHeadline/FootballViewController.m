...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = NSLocalizedString(@"Football Headlines", @"Football Headlines");
        self.tabBarItem.image = [UIImage imageNamed:@"second"];
        self.tabBarItem.title = @"Football";
...
- (void)refreshData{
    ...
    if ([[[DataService sharedService].messages objectForKey:kFootCount] intValue] > [[[DataService sharedService].messages objectForKey:kFootMsg] count] - 1) {
        self.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",[[[DataService sharedService].messages objectForKey:kFootCount] intValue] - [[[DataService sharedService].messages objectForKey:kFootMsg] count] + 1];
    }else{
...
- (void)viewDidLoad
{
    ...
    [downloader startDownload:kFootball withType:@"foot" page:page];
    [downloader release];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData) name:@"footMessage" object:nil];
...
{
    NSMutableArray *arr = [[DataService sharedService].messages objectForKey:kFootMsg];
    return arr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Message *message = [[[DataService sharedService].messages objectForKey:kFootMsg] objectAtIndex:indexPath.row];
    BOOL b = NO;
    ...
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kFootMsg] count]) {
...
{
	Message* msg = [[[DataService sharedService].messages objectForKey:kFootMsg] objectAtIndex:indexPath.row];
...
{
    if (indexPath.row + 1 == [[[DataService sharedService].messages objectForKey:kFootMsg] count]  && isMore){
        if ([[[DataService sharedService].messages objectForKey:kFootCount] intValue] > [[[DataService sharedService].messages objectForKey:kFootMsg] count] - 1) {
            page++;
            activity = [[MBProgressHUD alloc] initWithView:self.view];
            activity.center = self.view.center;
    ...
    }else if(indexPath.row + 1 < [[[DataService sharedService].messages objectForKey:kFootMsg] count]){
